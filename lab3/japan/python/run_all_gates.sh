...
CF_DISTRIBUTION_ID=E35DOVRWEUTMBX \
...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  if [[ -n "$cf_resource_arn" ]]; then
    waf_assoc_arn="$(aws cloudfront get-distribution --id "$CF_DISTRIBUTION_ID" --region "$CF_ACM_REGION" \
  --query "Distribution.DistributionConfig.WebACLId" --output text 2>/dev/null || echo "None")"

if [[ "$waf_assoc_arn" != "None" && -n "$waf_assoc_arn" ]]; then
  add_detail "PASS: WAF WebACL is associated with CloudFront (ARN=$waf_assoc_arn)."
