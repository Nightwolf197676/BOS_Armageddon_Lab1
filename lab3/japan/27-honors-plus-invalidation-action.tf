...
# Explanation: This is edo’s “break glass” lever — use it sparingly or the bill will bite.
# No native Terraform resource exists for CloudFront invalidations, so we use null_resource + local-exec
# to call the AWS CLI. Run this manually via: terraform apply -target=null_resource.edo_invalidate_index01
resource "null_resource" "edo_invalidate_index01" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      aws cloudfront create-invalidation \
        --distribution-id ${aws_cloudfront_distribution.edo_cf01.id} \
